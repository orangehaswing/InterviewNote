...
- **独立部署：**
- **技术选型灵活：** 技术选型是去中心化的。
- **容错：**  故障会被隔离在单个服务中。若设计良好，其他服务可通过重试、平稳退化等机制实现应用层面的容错。
...
Eureka 是一个服务中心，将所有的可以提供的服务都注册到它这里来管理，其它各调用者需要的时候去注册中心获取，然后再进行调用。自动具有了注册中心、负载均衡、故障转移的功能。
...
3. 服务调用者不可用：
   ...
   同步等待造成的资源耗尽，使用 **同步调用** 时，会产生大量的等待线程占用系统资源。一旦线程资源被耗尽，服务调用者提供的服务也将处于不可用状态。
